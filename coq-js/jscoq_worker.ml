...
type jscoq_answer =
  ...
  | GoalInfo  of stateid * richpp
  | CoqInfo   of string
  | Log       of level   * richpp
  | CoqOpt    of gvalue
  | Feedback  of feedback
  | CoqExn    of loc option * (stateid * stateid) option * richpp
  | JsonExn   of string
  [@@deriving yojson]
...
let jscoq_execute =
  ...
  | Init(lps, mods)     -> let iid = exec_init lps mods in
